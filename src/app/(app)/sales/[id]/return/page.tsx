...
async function processReturn(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    const totalAmt = lines.reduce((a, l) => a + l.qty * l.item.price, 0);
    ...
    if (sale.customerId) {
      await tx.customerLedger.create({
        data: {
          ...
          refId: ret.id,
          credit: totalAmt,
          notes: `Sale return ${sale.invoiceNo}`,
        },
      });
    }
  });
...
export default async function SaleReturnPage({
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`${t("sales.return")} - ${sale.invoiceNo}`}
        action={
          ...
          <form action={processReturn} className="space-y-4">
            ...
            <Table>
              <THead>
                <TR>
                  <TH>{t("common.name")}</TH>
                  <TH className="text-right">Sold {t("common.qty")}</TH>
